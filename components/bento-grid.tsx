...
export default function BentoGrid({ items = itemsSample }: BentoGridProps) {
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 p-4 max-w-7xl mx-auto" id="features">
            {items.map((item) => (
                <Link
                    href="#features"
                ...
                >
                    ...
                    >
                        ...
                        <CardContent className="relative space-y-2 p-4 pt-0">
                            ...
                            <p className="text-sm  leading-snug font-[425]">
                                {item.description}
                            </p>
                        </CardContent>
                            ...
                            <div className="flex items-center justify-between w-full">
                                <div className="flex items-center space-x-2 text-xs ">
                                    {item.tags?.map((tag) => (
                                        <span
                                            key={`${item.title}-${tag}`}
                                            className="px-2 py-1 rounded-md  backdrop-blur-xs transition-all duration-200"
                                        >
                                            #{tag}
                                        </span>
                                    ))}
                        ...
                        <div
                            className={cn(
                                "absolute inset-0 -z-10 rounded-xl p-px bg-linear-to-br from-transparent via-accent/70 to-transparent dark:via-white/10",
                                item.hasPersistentHover
                                    ? "opacity-100"
                                    : "opacity-0 group-hover:opacity-100",
                                "transition-opacity duration-300"
